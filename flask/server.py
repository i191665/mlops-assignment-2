...
@app.route('/')
def home():
  df = pd.read_csv(f'https://www.alphavantage.co/query?function=TIME_SERIES_{interval}_ADJUSTED&symbol={ticker}&datatype=csv&apikey={api_key}')
  ...
  return render_template('home.html', prices=prices, dates=dates, prediction=prediction)
...
def service():
  ...
  if form.validate_on_submit():
    ...
    return render_template('service.html', form=form, target=y, prediction=prediction, mae=mae)
